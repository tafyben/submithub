            ...
            @foreach($assignments as $assignment)
                <div class="border rounded p-4 shadow-sm bg-white">
                    ...
                    <p class="mt-2 text-sm">
                        Student: {{ $assignment->student->name }} ({{ $assignment->student->email }})
                    </p>
                    ...
                    <a href="{{ route('admin.assignments.show', $assignment) }}" class="mt-2 inline-block bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Process Assignment</a>
                </div>
            @endforeach
        </div>
    @else
        <p class="text-gray-500">No assignment requests found.</p>
    @endif
</x-app-layout>
